...
uninstall_gaming_dependencies() {
	...
	for pkg in "${packages[@]}"; do
		if yay -Qi "$pkg" &>/dev/null; then  # Check if package is installed
			if pactree -r "$pkg" 2>/dev/null | grep -qv "^${pkg}$"; then  # Check if no other packages require it
...
uninstall_amdgpu_driver() {
	local amd_pkgs=(mesa lib32-mesa vulkan-radeon lib32-vulkan-radeon vulkan-icd-loader lib32-vulkan-icd-loader lact)
	local to_remove=()
	...
	if [[ ${#to_remove[@]} -gt 0 ]]; then
		...
		echo -e "${YELLOW}No AMD GPU packages found to remove.${NC}"
	fi
}
...
cleanup() {
	sudo pacman -Syy
	echo -e "${YELLOW}Do you want to run a full system update? (y/n)${NC}"
